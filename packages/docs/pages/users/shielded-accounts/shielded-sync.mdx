...
Even though the sync operation is embedded in the commands that need it, it is
also possible to run the sync command directly:

```bash copy
  namadac shielded-sync
```

The arguments/options described in this document apply to this command as well.
